...
function ProjectCard({ title, description, tags, github, live, index }) {
  return (
    <motion.div
      ...
      whileHover={{ y: -4 }}
      className="bg-card rounded-xl p-6 gradient-border hover:glow transition-all duration-300 flex flex-col"
    >
      ...
      <div className="flex flex-wrap gap-2">
        {tags.map((tag) => (
          <span
            key={tag}
            className="font-mono text-xs text-accent bg-accent/10 px-3 py-1 rounded-full border border-accent/20"
          >
            {tag}
          </span>
        ))}
  ...
  return (
    <section id="projects" className="py-24 px-6 bg-surface/30">
      <div className="max-w-6xl mx-auto">
        ...
        >
          <p className="font-mono text-accent text-sm tracking-widest uppercase mb-3">Things I&apos;ve built</p>
